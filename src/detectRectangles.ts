...
export function detectRectangles(imageFile) {
    ...
    const threshold = img.adaptiveThreshold(255, cv.ADAPTIVE_THRESH_GAUSSIAN_C, cv.THRESH_BINARY_INV, 7, 2);
    ...
    contours.forEach(val => {
        img.drawRectangle(val.boundingRect(), new cv.Vec3(255, 0, 0), 3);
    })
    
    cv.imwrite('image.highlighted.jpg', img);
    
    return contours.map(val => val.hierarchy);
}
